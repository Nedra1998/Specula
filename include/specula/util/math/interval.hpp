...
namespace specula {
  ...
  public:
    ...
    SPECULA_CPU_GPU static Interval from_value_and_error(Float v, Float err) {
      Interval i{};
      if (err == 0) {
        i.low = i.high = v;
      } else {
    ...
    SPECULA_CPU_GPU [[nodiscard]] Float upper_bound() const { return high; }
    SPECULA_CPU_GPU [[nodiscard]] Float lower_bound() const { return low; }
    SPECULA_CPU_GPU [[nodiscard]] Float midpoint() const { return (low + high) / 2; }
    SPECULA_CPU_GPU [[nodiscard]] Float width() const { return high - low; }
    ...
    SPECULA_CPU_GPU [[nodiscard]] bool exactly(Float v) const { return low == v && high == v; }
    ...
    SPECULA_CPU_GPU Interval operator+(Float f) const { return *this + Interval(f); }
    SPECULA_CPU_GPU Interval operator-(Float f) const { return *this - Interval(f); }
    SPECULA_CPU_GPU Interval operator*(Float f) const {
      if (f > 0.f) {
        return {mul_round_down(f, low), mul_round_up(f, high)};
      } else {
        return {mul_round_down(f, high), mul_round_up(f, low)};
      }
    }
    SPECULA_CPU_GPU Interval operator/(Float f) const {
      if (f == 0) {
      ...
      if (f > 0.f) {
        return {div_round_down(low, f), div_round_up(high, f)};
      } else {
        return {div_round_down(high, f), div_round_up(low, f)};
      }
    }
    ...
    SPECULA_CPU_GPU Interval operator*(Interval i) const {
      Float low_prod[4] = {mul_round_down(low, i.low), mul_round_down(high, i.low),
                           mul_round_down(low, i.high), mul_round_down(high, i.high)};
      Float high_prod[4] = {mul_round_up(low, i.low), mul_round_up(high, i.low),
                            mul_round_up(low, i.high), mul_round_up(high, i.high)};
      return {std::min({low_prod[0], low_prod[1], low_prod[2], low_prod[3]}),
              std::max({high_prod[0], high_prod[1], high_prod[2], high_prod[3]})};
    }
    SPECULA_CPU_GPU Interval operator/(Interval i) const {
      if (in_range(0, i)) {
        return {-INFINITY, INFINITY};
  ...
  SPECULA_CPU_GPU inline Interval mul_pow2(Interval i, Float s) {
    ...
    return {
        std::min(i.lower_bound() * s, i.upper_bound() * s),
  ...
  SPECULA_CPU_GPU inline Interval operator*(Float f, Interval i) {
    if (f > 0.f) {
      return {mul_round_down(f, i.lower_bound()), mul_round_up(f, i.upper_bound())};
    } else {
      return {mul_round_down(f, i.upper_bound()), mul_round_up(f, i.lower_bound())};
    }
  }
  SPECULA_CPU_GPU inline Interval operator/(Float f, Interval i) {
    if (in_range(0, i)) {
    ...
    if (f > 0.f) {
      return {div_round_down(f, i.upper_bound()), div_round_up(f, i.lower_bound())};
    } else {
      return {div_round_down(f, i.lower_bound()), div_round_up(f, i.upper_bound())};
    }
  }
  ...
  SPECULA_CPU_GPU inline Interval fma(Interval a, Interval b, Interval c) {
    Float low = std::min({fma_round_down(a.lower_bound(), b.lower_bound(), c.lower_bound()),
    ...
    Float high = std::max({fma_round_up(a.lower_bound(), b.lower_bound(), c.upper_bound()),
  ...
  SPECULA_CPU_GPU inline Interval difference_of_products(Interval a, Interval b, Interval c,
    ...
    int ablow_index = ablow == ab[0] ? 0 : (ablow == ab[1] ? 1 : (ablow == ab[2] ? 2 : 3));
    int abhigh_index = abhigh == ab[0] ? 0 : (abhigh == ab[1] ? 1 : (abhigh == ab[2] ? 2 : 3));
    ...
    int cdlow_index = cdlow == cd[0] ? 0 : (cdlow == cd[1] ? 1 : (cdlow == cd[2] ? 2 : 3));
    int cdhigh_index = cdhigh == cd[0] ? 0 : (cdhigh == cd[1] ? 1 : (cdhigh == cd[2] ? 2 : 3));
  ...
  SPECULA_CPU_GPU inline Interval acos(Interval i) {
    ...
    return {std::max<Float>(0, next_float_down(low)), next_float_up(high)};
  }
