#ifndef SPECULA_UTIL_MATH_BIT_OPERATIONS_HPP
#define SPECULA_UTIL_MATH_BIT_OPERATIONS_HPP
...
namespace specula {
  SPECULA_CPU_GPU inline uint32_t reverse_bits_32(uint32_t n) {
  ...
  inline SPECULA_CPU_GPU uint32_t encode_morton3(float x, float y, float z) {
    ...
    return (left_shift3(static_cast<uint32_t>(z)) << 2) |
           (left_shift3(static_cast<uint32_t>(y)) << 1) | left_shift3(static_cast<uint32_t>(x));
  }
...
#endif // SPECULA_UTIL_MATH_BIT_OPERATIONS_HPP
