...
TEST_CASE("MonotonicBufferResource", "[util][pstd]") {
  SECTION("No overlap") {
    TrackingResource tr;
    pmr::monotonic_buffer_resource mb(1024, &tr);
    specula::Allocator alloc(&mb);
    specula::Rng rng;
    struct Span {
      char *ptr;
      size_t size;
    ...
    for (int i = 0; i < 10'000; ++i) {
      ...
      spans.push_back(Span{.ptr = p, .size = size});
    }
  }
}
